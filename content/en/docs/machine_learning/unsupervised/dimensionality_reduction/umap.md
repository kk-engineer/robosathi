...
{{< panel color="navy" title="UMAP" id="umap">}}
**Non-linear** dimensionality reduction technique to visualize high-dimensional data (like images, gene expressions) 
in a lower-dimensional space (typically 2D or 3D), preserving its underlying structure and relationships.
...
_Note_: Similar to t-SNE but often **faster** and **better** at maintaining global structure.
{{< /panel >}}
...
_Note_: Fuzzy means instead of using binary 0/1, we use weights in the range [0,1] for each edge.
{{< /panel >}}
...
_Note_: This ensures that the closest point always gets a weight of 1, preserving local structure.
{{< /panel >}}
...
_Note_: The parameters 'a' and 'b' are typically fixed based on the 'min_dist' user parameter 
(e.g. min_dist = 0.1, then a≈1,577, b≈0.895).
{{< /panel >}}

{{< panel color="green" title="Optimization" >}}
Unlike t-SNE's KL divergence, UMAP minimizes the **cross-entropy** between the high-dimensional weights \(W_{ij}\) 
...
**Goal** : Reduce overall _Cross Entropy Loss_.
