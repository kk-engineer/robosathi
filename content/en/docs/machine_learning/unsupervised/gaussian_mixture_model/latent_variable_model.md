...
{{< panel color="orange" title="Gaussian Mixture Model (GMM)" id="gmm">}}
...
_Note_: \(\pi _{k}\approx \frac{\text{Number\ of\ points\ in\ cluster\ }k}{\text{Total\ number\ of\ points\ }(N)}\)
...
_Note_: We did not record **gender**, so it is '**hidden**' or '**latent**'.
{{< /panel >}}
...
{{< panel color="gold" title="GMM as Latent Variable Model" >}}
GMM is a latent variable model, meaning each data point \(\mathbf{x}_{i}\) is assumed to have an associated unobserved (latent) variable
\(z_{i}\in \{1,\dots ,K\}\) indicating which component generated it.

_Note_: We observe the data point, but we do not observe which cluster it belongs to (\(z_i\)).
{{< /panel >}}
...
_Note_: The **challenge** lies in **estimating** **both** the parameters of the **Gaussians** and the values 
...
_Note_: This allows the logarithm to act directly on the exponential term of the Gaussian, leading to simple linear equations.
{{< /panel >}}
